...
class Scene: SKScene {
    ...
    private func drawBox(_ box: CGRect) {
        let actualPoint = CGPoint(x: CGFloat(box.minX) * self.view!.frame.width, y: CGFloat(box.minY) * self.view!.frame.height)
    ...
    private func createButton(x: CGFloat, y: CGFloat, title: String, action: Selector) -> UIButton {
        let button = UIButton(frame: CGRect(x: x, y: y, width: 90, height: 40))
    ...
    private func presentView(at rect: CGRect) {
        let transferButton = createButton(x: rect.minX, y: rect.maxY, 
                                          title: "Transfer", action: #selector(transferAction))
        let requestButton = createButton(x: rect.minX, y: transferButton.frame.maxY, 
                                         title: "Request", action: #selector(requestAction))
        let addButton = createButton(x: rect.minX, y: requestButton.frame.maxY, 
    ...
    private func findFaces() {
        ...
        var uiImage = resizeImage(image: uiImageOld, newWidth: self.view!.frame.width, newHeight: self.view!.frame.height)
